 ...
 * Output: supabase/seed/bulk-upload-2-badran.sql
 */
...
// Valid endpoint: >= 75 (new nodes we insert) or 242/398 (merge to 021/022).
// IDs 1-74 in JSON map to UUIDs that COLLIDE with original seed (e.g. 46→Salam Zabalawi, 74→Samira Zabalawi).
// We must NOT create links referencing 1-74 - they would wrongly connect to original tree members.
function isValidEndpoint(id) {
  return id >= 75 || id === MERGE_242_TO_021 || id === MERGE_398_TO_022;
}
...
const newLinks = newLinksRaw.map((l) => {
  const src = l.source === MERGE_242_TO_021 ? ORIGINAL_021 : l.source === MERGE_398_TO_022 ? ORIGINAL_022 : idToUuid(l.source);
  const tgt = l.target === MERGE_242_TO_021 ? ORIGINAL_021 : l.target === MERGE_398_TO_022 ? ORIGINAL_022 : idToUuid(l.target);
